...
def flux_fraction_contaminant(ang_sep, s, d_th=0.000005):
    ...
    3b-10 from Biser & Millman (1965), which is a double converging sum with
    infinite limits, given by:
    $f_{\rm bg} = e^{-t} \sum_{n=0}^{n\to{\infty}} \\
    {\Bigg\{\frac{t^{n}}{n!}\bigg[1-e^{-s}\sum_{k=0}^{n}{\frac{s^{k}}{k!}}}
    \bigg]\Bigg\}$
...
def contamination(t_targets, LC_con, Rad=1.0, n_cont=5):
    ...
    uses an analytical formula (Biser & Millman 1965,
    https://books.google.co.uk/books?id=5XBGAAAAYAAJ -- equation 3b-10) to
