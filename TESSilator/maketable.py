...
def tableFromSimbad(input_names):
...
def tableFromTable(input_table, name_is_source_id=0):
    ...
    input_table : `astropy.table.Table'
        ...
        The column headers are not necessary, but the data types must be
        readable as:
        str, float, float, float, float (respectively).
...
def getGaiaData(gaia_table, name_is_source_id=0):
    ...
    THE USER MUST SUPPLY A TABLE WITH COMMA-SEPARATED VARIABLES!
    ...
    2) A table with sky-coordinates in either the 'icrs' (default),'galactic',
       or 'ecliptic' system.
    ...
    tbl : `astropy.table.Table`
        The table ready for TESSilator analysis.
    '''
